...
class NeRFRenderer(torch.nn.Module):
    """
    定义nerf的可微渲染器
    :param n_coarse 粗采样个数
    :param n_fine 细采样个数
    :param n_fine_depth number of expected depth samples
    :param noise_std 对体密度添加的噪声. We do not use it
    :param depth_std 对深度添加的噪声
    :param eval_batch_size 评估时光线的数量
    :param white_bkgd if true, background color is white; else black
    :param lindisp 如果为true则不是按照距离抽样点
    ...
    def sample_coarse(self, rays):
        ...
        if not self.lindisp:  # 获取到每个采样点的深度
    ...
    def sample_fine(self, rays, weights):
        ...
        基于权重weights的采样
        :param rays ray [origins (3), directions (3), near (1), far (1)] (B, 8)
        :param weights (B, Kc) Kc指的是coarse的采样个数
        :return (B, Kf-Kfd) Kf
        """
        device = rays.device
        B = rays.shape[0]
        ...
        cdf = torch.cumsum(pdf, -1)  # (B, Kc) cumsum计算累计和
        ...
        inds = torch.clamp_min(inds, 0.0) # 把inds中小于0的全部设置为0
    ...
    def sample_fine_depth(self, rays, depth):
        ...
        z_samp = torch.max(torch.min(z_samp, rays[:, -1:]), rays[:, -2:-1]) # 看深度是否小于near,大于far,否则就将其固定在这之间
        return z_samp
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            B, K = z_samp.shape # B = 4个场景，每个场景有一张图片，每个图片里抽样128条光线，共有B=4X128=512条，每条光线粗采样点K=64个

            deltas = z_samp[:, 1:] - z_samp[:, :-1]  # (B, K-1)
            #  if far:
            #      delta_inf = 1e10 * torch.ones_like(deltas[:, :1])  # infty (B, 1)
            delta_inf = rays[:, -1:] - z_samp[:, -1:] # 最后一段delta的距离
            deltas = torch.cat([deltas, delta_inf], -1)  # (B, K(64))

            # (B, K, 3) points = o + td,t为深度
            ...
            if sb > 0:
                points = points.reshape(
                    sb, -1, 3
                )  # (SB, B'*K=128x64, 3) B' is real ray batch size,K是每条光线的采样点个数
                eval_batch_size = (self.eval_batch_size - 1) // sb + 1 # 每个场景要eval的采样点数,25000个？
                eval_batch_dim = 1
            else:
                eval_batch_size = self.eval_batch_size
                eval_batch_dim = 0

            split_points = torch.split(points, eval_batch_size, dim=eval_batch_dim) # 这里全部取
            if use_viewdirs:
                dim1 = K
                viewdirs = rays[:, None, 3:6].expand(-1, dim1, -1)  # (B, K, 3)
                if sb > 0:
                    viewdirs = viewdirs.reshape(sb, -1, 3)  # (SB, B'*K = 128x64, 3)
                else:
            ...
            if self.white_bkgd:
                # White background
                pix_alpha = weights.sum(dim=1)  # (B), pixel alpha是不透明度么？
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            rays = rays.reshape(-1, 8)  # (SB * B, 8)，将多个场景的光线合并？

            z_coarse = self.sample_coarse(rays)  # 获取到每个采样点的深度
            ...
            if self.using_fine: # 进行细采样？
                all_samps = [z_coarse]
                if self.n_fine - self.n_fine_depth > 0: # 减去按照深度选取细采样点的个数
                    all_samps.append(
                        self.sample_fine(rays, coarse_composite[0].detach())
                    )  # (B, Kf - Kfd) 获得到细采样点的深度信息
                if self.n_fine_depth > 0: # 按照深度细采样点
                    all_samps.append(
    ...
    def _format_outputs(
        ...
        if superbatch_size > 0:
            rgb = rgb.reshape(superbatch_size, -1, 3) # 将rgb,depth,weights重新reshape成(场景数，光线数，xxx)
    ...
    def bind_parallel(self, net, gpus=None, simple_output=False):
        ...
        :param gpus 需要数据并行的GPU列表，如果没有多GPU则为[0]
