...
class PixelNeRFNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False):
        ...
        """
        super().__init__()
        self.encoder = make_encoder(conf["encoder"]) # 用resnet34和双线性插值做encoder
        self.use_encoder = conf.get_bool("use_encoder", True)  # Image features?

        self.use_xyz = conf.get_bool("use_xyz", False) # 是否使用3维坐标

        assert self.use_encoder or self.use_xyz  # Must use some feature..

        # 是否将Z对齐到标准的空间
        # 对于所有的物体的z值都会集中在0，不管相机距离物体中心的距离多少，相当于把世界坐标系固定在物体中心？
        # 只在Shapenet型数据中有效
        ...
        self.use_code = conf.get_bool("use_code", False)  # 是否对xyz使用位置编码(做)
        self.use_code_viewdirs = conf.get_bool(
            "use_code_viewdirs", True
        )  # 是否对单位化的光线方向做位置编码(默认不做)
        ...
        if self.use_code and d_in > 0:
            ...
            self.code = PositionalEncoding.from_conf(conf["code"], d_in=d_in) # d_out = (3 x (6x2) + 3 = 39)
            d_in = self.code.d_out
        if self.use_viewdirs and not self.use_code_viewdirs:
            # 不对viewdir做位置编码，但是输入要包含这个特征
            d_in += 3

        if self.use_global_encoder: # 暂时用不到
        ...
        self.mlp_coarse = make_mlp(conf["mlp_coarse"], d_in, d_latent, d_out=d_out) # todo d_in = 39 + 3 = 42
    ...
    def encode(self, images, poses, focal, z_bounds=None, c=None):
        ...
        if len(images.shape) == 5:
            ...
            self.num_views_per_obj = images.size(1) # 每个三维重建物体的视图个数？
            images = images.reshape(-1, *images.shape[2:])
            poses = poses.reshape(-1, 4, 4)
        else:
            self.num_views_per_obj = 1

        self.encoder(images) # 对图片提取特征
        rot = poses[:, :3, :3].transpose(1, 2)  # (B, 3, 3)
        trans = -torch.bmm(rot, poses[:, :3, 3:])  # (B, 3, 1) 获得世界坐标系下的相机中心位置？
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False): # todo 这里还要看
        """
        Predict (r, g, b, sigma) at world space points xyz.
        Please call encode first!
        :param xyz (SB, B, 3)
        SB is batch of objects
        B 是每条光线采样点的个数，128x64=8192
        ...
        with profiler.record_function("model_inference"):
            ...
            if self.d_in > 0:
                ...
                if self.use_viewdirs:
                    ...
                    viewdirs = repeat_interleave(viewdirs, NS)  # 重复插值？(SB*NS, B, 3, 1)
                    viewdirs = torch.matmul(
                        self.poses[:, None, :3, :3], viewdirs
                    )  # (SB*NS, B, 3, 1)
                    viewdirs = viewdirs.reshape(-1, 3)  # (SB*B*NS, 3)
                    z_feature = torch.cat(
    ...
    def load_weights(self, args, opt_init=False, strict=True, device=None):
        ...
        elif not opt_init: # 如果checkpoints读取失败，打印警告
            warnings.warn(
                (
