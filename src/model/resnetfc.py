...
class ResnetFC(nn.Module):
    ...
    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        """
        :param zx (..., d_latent + d_in) 512 + 42 = 554
        :param combine_inner_dims Combining dimensions for use with multiview inputs.
        Tensor will be reshaped to (-1, combine_inner_dims, ...) and reduced using combine_type
        on dim 1, at combine_layer
        """
        with profiler.record_function("resnetfc_infer"):
            assert zx.size(-1) == self.d_latent + self.d_in # d_latent可能是像素的特征？
            if self.d_latent > 0:
                z = zx[..., : self.d_latent] # 对应论文图中的大W部分
                x = zx[..., self.d_latent :] # 对应图中的（位置编码，方向向量）
            else:
                x = zx
            if self.d_in > 0:
                x = self.lin_in(x) # 位置编码首先升维到512
            else:
                x = torch.zeros(self.d_hidden, device=zx.device)

            for blkid in range(self.n_blocks): # n_blocks为5，
                if blkid == self.combine_layer: # 如果是第三层就进行全局的平均池化
                    ...
                    #      )
                    #  else:
                    x = util.combine_interleaved( # 将中间向量V用均值归一化?
                        x, combine_inner_dims, self.combine_type
                    )

                if self.d_latent > 0 and blkid < self.combine_layer:
                    tz = self.lin_z[blkid](z) # block里对于像素特征的那个fc
                    if self.use_spade:
            ...
            out = self.lin_out(self.activation(x)) # 最后一层
            return out # (4,8192,4) 得到每个场景的采样点的颜色（BS，128x64,4）
