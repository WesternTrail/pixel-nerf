...
dset, val_dset, _ = get_split_dataset(args.dataset_format, args.datadir) # 根据dataset_format初始化数据集，dset=trainset,_代表test_set
print(
    "dset z_near {}, z_far {}, lindisp {}".format(dset.z_near, dset.z_far, dset.lindisp)
)

net = make_model(conf["model"]).to(device=device) # 创建pixelnerf的f1,f2网络
...
nviews = list(map(int, args.nviews.split())) # 用一张视图去做三维重建？


class PixelNeRFTrainer(trainlib.Trainer):
    def __init__(self):
        super().__init__(net, dset, val_dset, args, conf["train"], device=device) # 对父类进行初始化
        self.renderer_state_path = "%s/%s/_renderer" % (
            self.args.checkpoints_path,
            self.args.name,
        )

        self.lambda_coarse = conf.get_float("loss.lambda_coarse") # 训练损失函数的参数?
        self.lambda_fine = conf.get_float("loss.lambda_fine", 1.0)
        print(
            "lambda coarse {} and fine {}".format(self.lambda_coarse, self.lambda_fine)
        )
        self.rgb_coarse_crit = loss.get_rgb_loss(conf["loss.rgb"], True) # 粗网络rgb的loss函数
        fine_loss_conf = conf["loss.rgb"]
        if "rgb_fine" in conf["loss"]: # fine网络是否要优化rgb
            print("using fine loss")
            fine_loss_conf = conf["loss.rgb_fine"]
        self.rgb_fine_crit = loss.get_rgb_loss(fine_loss_conf, False) # 细网络rgb的loss函数也是mesloss

        if args.resume: # 是否接着上一次训练
            if os.path.exists(self.renderer_state_path):
                renderer.load_state_dict(
                    torch.load(self.renderer_state_path, map_location=device)
                )

        self.z_near = dset.z_near # multi-obj是4-9的渲染空间，srn数据集是0.8-1.8
    ...
    def calc_losses(self, data, is_train=True, global_step=0):
        ...
        all_poses = data["poses"].to(device=device)  # (SB, NV, 4, 4),验证集为空
        ...
        if curr_nviews == 1:
            image_ord = torch.randint(0, NV, (SB, 1)) # 从batch=4中每个场景的50张图片中随机选取一张的索引
        else:
            image_ord = torch.empty((SB, curr_nviews), dtype=torch.long)
        for obj_idx in range(SB): # 遍历batch_size个场景，从一张图片中抽样128条光线，将信息存放到列表all_rgb_gt中
            if all_bboxes is not None: # 获取到框住前景的bbx
            ...
            images_0to1 = images * 0.5 + 0.5 # 这个是在干嘛？

            cam_rays = util.gen_rays(
                poses, W, H, focal, self.z_near, self.z_far, c=c
            )  # 生成一个场景的所有视角的光线(NV, H, W, 8(cam_centers, cam_raydir, cam_nears, cam_fars))
            rgb_gt_all = images_0to1 # (NV, H, W, 3)
            rgb_gt_all = (
                rgb_gt_all.permute(0, 2, 3, 1).contiguous().reshape(-1, 3)
            )  # (819200,3)

            if all_bboxes is not None:
                pix = util.bbox_sample(bboxes, args.ray_batch_size) # ray_batch_size：每次从一个场景的50个bbx（框住了前景）里面抽样128条光线，pix：(num_ray,imageid+y+x)
                pix_inds = pix[..., 0] * H * W + pix[..., 1] * W + pix[..., 2] # 获取到每条光线在819200的索引
            else:
                pix_inds = torch.randint(0, NV * H * W, (args.ray_batch_size,)) # 直接从当前场景抽样128条光线

            rgb_gt = rgb_gt_all[pix_inds]  # (ray_batch_size, 3) 每个场景只会抽样128条光线
            rays = cam_rays.view(-1, cam_rays.shape[-1])[pix_inds].to(
                device=device
            )  # (ray_batch_size, 8),获取抽样光线的方向，位置，近远平面
        ...
        image_ord = image_ord.to(device) # 训练只会从一个场景的50张图片中选择一张图片作为源图像送入到encoder中去
        src_images = util.batched_index_select_nd(
            all_images, image_ord
        )  # (SB, NS, 3, H, W) 获取源图像
        src_poses = util.batched_index_select_nd(all_poses, image_ord)  # (SB, NS, 4, 4)，获取源图像的位姿
        ...
        render_dict = DotMap(render_par(all_rays, want_weights=True,)) # todo 数据并行化进行render
    ...
    def vis_step(self, data, global_step, idx=None):
        if "images" not in data:
            return {}
        if idx is None:
            batch_idx = np.random.randint(0, data["images"].shape[0]) # 从0-4中随机选一个场景
        else:
            print(idx)
            batch_idx = idx
        ...
        views_src = np.sort(np.random.choice(NV, curr_nviews, replace=False)) # 随机从251个视角选择一个视角作为源视角
        view_dest = np.random.randint(0, NV - curr_nviews) # 其他一个视角作为目标视角
        for vs in range(curr_nviews):
            view_dest += view_dest >= views_src[vs] # 如果目标视角的索引大于参考视角，则加一？
        ...
        gt = images_0to1[view_dest].permute(1, 2, 0).cpu().numpy().reshape(H, W, 3) # GT是目标视角
        with torch.no_grad():
            test_rays = cam_rays[view_dest]  # (H, W, 8) ，光线是从目标视角发出的
            test_images = images[views_src]  # (NS, 3, H, W)
            net.encode(
                test_images.unsqueeze(0), # 对源视角进行提取特征
        ...
        alpha_coarse_cmap = util.cmap(alpha_coarse_np) / 255 # todo 不知道cmap这是啥？
        ...
        if using_fine:
            ...
            vis_fine = np.hstack(vis_list) # 变为tensor
            vis = np.vstack((vis_coarse, vis_fine))
            rgb_psnr = rgb_fine_np
        else:
            rgb_psnr = rgb_coarse_np

        psnr = util.psnr(rgb_psnr, gt) # 可视化阶段计算psnr
        vals = {"psnr": psnr}
        print("psnr", psnr)
